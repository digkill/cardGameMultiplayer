...
export const UI = () => {
    ...
    switch (phase) {
        case 'cards':
            label = 'Select the card you want to platy'
            break
        case 'playerChoice':
            label = currentPlayer.id === me.id
                ? 'Select the player you want to punch' : `${currentPlayer?.state.profile?.name} is going to punch someone`
            break
        case 'playerAction':
            switch (currentCard) {
                case 'punch':
                    ...
                    ? `${currentPlayer?.state.profile?.name} is punching ${target?.state.profile?.name}`
                        : `${currentPlayer?.state.profile} failed punching ${target?.state.profile?.name}`
                    break
                case 'grab':
                    label = actionSuccess
